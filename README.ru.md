...
| `execute_maya_code` | Python внутри Maya — `cmds` / `om` / `omui` / `mel` / `mutils` прединжектированы, stdout/stderr ловятся, опциональная переменная `result` возвращается (JSON-safe), `undo_chunk=True` оборачивает вызов в один undo-шаг |
...
| `get_console_log` | кольцевой буфер stdout/stderr по исполнениям (`last_n` / `filter` / `stream`) |
